...
async function fetchUnsplash(tag, count) {
...
async function fetchPixabay(tag, count) {
...
async function fetchPexels(tag, count) {
...
export async function fetchFromGallery(galleryUrl, tag, count) {
    ...
    if (hostname === 'unsplash.com') {
        if (!process.env.UNSPLASH_ACCESS_KEY)
            throw new Error('Unsplash requires UNSPLASH_ACCESS_KEY (free at unsplash.com/developers)');
        return fetchUnsplash(tag, count);
    }
    if (hostname === 'pixabay.com') {
        if (!process.env.PIXABAY_API_KEY)
            throw new Error('Pixabay is protected by Cloudflare and requires PIXABAY_API_KEY (free at pixabay.com/api/docs)');
        return fetchPixabay(tag, count);
    }
    if (hostname === 'pexels.com') {
        if (!process.env.PEXELS_API_KEY)
            throw new Error('Pexels requires PEXELS_API_KEY (free at pexels.com/api)');
