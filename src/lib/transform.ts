...
import type { ServeQuery } from '../types.js'
...
export async function transformImage(
  ...
  if (query.w || query.h) {
    pipeline = pipeline.resize(
      query.w ? parseInt(query.w, 10) : undefined,
      query.h ? parseInt(query.h, 10) : undefined,
  ...
  if (fmt === 'webp') {
    ...
    if (originalMime === 'image/jpeg') pipeline = pipeline.jpeg({ quality })
    else if (originalMime === 'image/webp') pipeline = pipeline.webp({ quality })
  }
