...
export async function analyzeWithAI(
...
): Promise<{ tags: string[]; category: Category }> {
  ...
  try {
    ...
    else if (buffer[0] === 0x52 && buffer[4] === 0x57) mediaType = 'image/webp'
    ...
    if (!match) return { tags: existingTags, category: existingCategory ?? '其他' }

    const parsed = JSON.parse(match[0])
    const aiTags: string[] = Array.isArray(parsed.tags) ? parsed.tags.map(String) : []
    const aiCategory: Category = isValidCategory(parsed.category) ? parsed.category : '其他'
